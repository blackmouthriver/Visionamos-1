...
const ModalRespuesta = ({ titleody, titleButom1, titleButom2, funcionButton1, funcionButton2, id, contenido1, contenido2, contenido3, calseModalboton, calseModaltitle }) => {
    ...
    const borrarComprobante = async () => {
        const repuesta = await axios.put(`http://localhost:3001/comprobantes/borrar/${id}`);
        if (repuesta.status === 200) {

            history.push("/comprobantes/confirmacion");
    ...
    const onclickReturn = () => {
        ...
        history.push("/comprobantes/crear");
    }
